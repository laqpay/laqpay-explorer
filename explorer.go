...
* LAQPAY_ADDR - The laqpay node's address. You must include a scheme. Defaults to http://127.0.0.1:6420

CLI Options:
* -api-only - Don't serve static content from ./dist, only proxy the laqpay node
...
const (
	...
	defaultLaqpayAddr  = "http://127.0.0.1:6420"

	// timeout for requests to the backend laqpay node
	laqpayRequestTimeout = time.Second * 30
...
var (
	...
	laqpayAddr  *url.URL // override with envvar LAQPAY_ADDR.  Must have scheme, e.g. http://
...
func init() {
	...
	laqpayAddrString := os.Getenv("LAQPAY_ADDR")
	if laqpayAddrString == "" {
		laqpayAddrString = defaultLaqpayAddr
	}

	origURL, err := url.Parse(laqpayAddrString)
	if err != nil {
		log.Println("LAQPAY_ADDR must have a scheme, e.g. http://")
		log.Fatalln("Invalid LAQPAY_ADDR", laqpayAddrString, err)
	}

	if origURL.Scheme == "" {
		log.Fatalln("LAQPAY_ADDR must have a scheme, e.g. http://")
	}
...
func buildLaqpayURL(path string, query url.Values) string {
	rawQuery := ""
	if query != nil {
	...
	u := &url.URL{
		Scheme:   laqpayAddr.Scheme,
		Host:     laqpayAddr.Host,
		Path:     path,
		RawQuery: rawQuery,
	}
...
type APIEndpoint struct {
	...
	LaqpayPath    string   `json:"laqpay_path"`
...
func (s APIEndpoint) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	laqpayURL := buildLaqpayURL(s.LaqpayPath, query)

	log.Printf("Proxying request %s to laqpay node %s with timeout %v", r.URL.String(), laqpayURL, laqpayRequestTimeout)

	c := &http.Client{
		Timeout: laqpayRequestTimeout,
	}

	resp, err := c.Get(laqpayURL)
	if err != nil {
		msg := "Request to laqpay node failed"
		log.Println("ERROR:", msg, laqpayURL)
	...
	if s.ExplorerPath == "/api/coinmarketcap" {
		...
		if err := json.NewDecoder(resp.Body).Decode(&cs); err != nil {
			...
			log.Println("ERROR:", msg, laqpayURL, err)
			http.Error(w, msg, http.StatusInternalServerError)
			return
		}
	...
	if n, err := io.Copy(w, resp.Body); err != nil {
		msg := "Copying response from laqpay node to client failed"
		if n != 0 {
...
var apiEndpoints = []APIEndpoint{
...

	{
		...
		LaqpayPath:    "/api/v1/blockchain/metadata",
...

	{
		...
		ExampleRequest: "https://explorer.laqpay.com/api/blocks?start=1891&end=1892",
		ExampleResponse: `{
    "blocks": [
        {
            "header": {
	...
	},
	{
		...
		LaqpayPath:    "/api/v1/addresscount",
	...
	},
	{
		...
		LaqpayPath:    "/api/v1/transactions",
...
<title>Laqpay Explorer API Documentation</title>
<style type="text/css">
...
<h1>Laqpay Explorer API Documentation</h1>

<div>
<p>
<p>The Laqpay Explorer API proxies a subset of a Laqpay node's API.</p>
<p>All endpoints start with /api</p>
<p>Further information about an endpoint can be found at the Laqpay repo.</p>
<p>Laqpay Github:<a href="https://github.com/laqpay/laqpay">https://github.com/laqpay/laqpay</a>.</p>
<p>Laqpay Explorer Github: <a href="https://github.com/laqpay/laqpay-explorer">https://github.com/laqpay/laqpay-explorer</a></p>
</p>
</div>
...
{{ if .LaqpayPath }}
<p>Internal laqpay node path:</p>
<p class="example"><code class="inline">{{ .LaqpayPath }}</code></p>
{{ end }}
...
func main() {
	...
	for _, e := range apiEndpoints {
		...
		log.Printf("%s proxied to %s with args %v", e.ExplorerPath, e.LaqpayPath, e.QueryArgs)
	}
	...
	log.Printf("Running laqpay explorer on http://%s", explorerHost)
